import pandas as pd
from modules.ranker.game import Game
...
def calc_stats(match_data: pd.DataFrame, player_data: pd.DataFrame):
    ...
    for keys, game_data in match_data.groupby(
        ["date", "set", "away_team", "home_team"]
    ):
        ...
        away_team_players = game_data.loc[
            game_data["team"] == game_data.away_team, "player"
        ].unique()
        home_team_players = game_data.loc[
            game_data["team"] == game_data.home_team, "player"
        ].unique()
        ...
        for play in game_data.itertuples(index=False):
            ...
            if play.team == play.away_team:
                for player in away_team_players:
                    game.player_data.loc[
                        player_data["player"] == player, f"{play.event}_opportunity"
                    ] += 1
                    player_data.loc[
                        player_data["player"] == player, f"{play.event}s"
                    ] += 1 / len(away_team_players)

                player_data.loc[
                    player_data["player"] == play.player, f"{play.event}s"
                ] += 1 / len(away_team_players)
                game.player_data.loc[
                    player_data["player"] == play.player, f"{play.event}s"
                ] += 1
            ...
            match play.event:
                case "Serve":
                    if len(actions.keys()) > 0:
                        ...
                        game.same_team_sets_from_serve[serve_team] += actions[
                            f"{serve_team}_sets"
                        ]
                        ...
                        game.opp_team_attacks_from_serve[receive_team] += actions[
                            f"{receive_team}_attacks"
                        ]
                    actions = {
...
def main():
    ...
    calc_stats(pd.read_csv(f"{PACKAGE_ROOT}/Data/test/test_game_1.csv"), players)
